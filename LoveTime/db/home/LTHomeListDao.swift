...
struct LTHomeListDao {
    ...
    func queryHomeListAll() async -> [LTHomeListModel] {
        guard let database = LTDataBaseTool.default.base else {
            return []
        }
        
//        let condion: Condition = LTHomeListModel.Properties.typeName == typeName
        let result: [LTHomeListModel]? = try? database.getObjects(fromTable: LT_LOVE_RECORD_TABLE_NAME, orderBy: [LTHomeListModel.Properties.create_Time.asOrder(by: .descending)])
        if let r = result {
            return r
        }
        return []
